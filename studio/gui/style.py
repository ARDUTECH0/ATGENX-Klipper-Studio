# -*- coding: utf-8 -*-
"""Dark theme. One accent colour drives the whole window and the icon set."""
from ..appdata import load_settings, save_settings

DEFAULT_ACCENT = "#e5484d"
PRESETS = [("Red", "#e5484d"), ("Blue", "#2f81f7"), ("Green", "#2ea043"),
           ("Orange", "#e8850d"), ("Purple", "#a371f7"), ("Teal", "#1f9c9c")]


def accent():
    return load_settings().get("accent") or DEFAULT_ACCENT
...
def lighter(hex_color, amount=40):
    c = hex_color.lstrip("#")
    rgb = tuple(min(255, int(c[i:i + 2], 16) + amount) for i in (0, 2, 4))
...
QTreeWidget::item:selected, QListWidget::item:selected {{ background: {accent}33; color: #e6edf3; }}
...
QListWidget#nav::item:selected {{ background: {accent}33; color: #e6edf3; border: none; }}
...
QPushButton#card:hover {{ border-color: {accent}; background: #1a2230; }}
...
QGroupBox::title {{ subcontrol-origin: margin; padding: 0 8px; color: {light}; font-weight: 600; }}
...
QLineEdit:focus, QSpinBox:focus, QDoubleSpinBox:focus, QComboBox:focus {{ border-color: {light}; }}
QComboBox QAbstractItemView {{ background: #161b22; selection-background-color: {accent}55; }}
QPushButton {{ background: #21262d; border: 1px solid #30363d; border-radius: 7px; padding: 7px 16px; }}
QPushButton:hover {{ border-color: {light}; }}
...
QFrame#featureCard:hover {{ border-color: {accent}; }}
...
QLabel#subHead {{ color: {light}; font-weight: 600; padding: 4px 0 0; }}
...
QPushButton#link {{ background: transparent; border: none; color: {light}; padding: 2px 0; text-align: left; }}
QPushButton#link:hover {{ color: #79c0ff; text-decoration: underline; }}
"""


def style(color=None):
    color = color or accent()
    return TEMPLATE.format(accent=color, light=lighter(color))
